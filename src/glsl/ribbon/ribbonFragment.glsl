...
void main() {
    ...
    directLight += pointLight(
        vec3(0.85, 0.85, 0.9), // Light color
        1.0,                 // Light intensity,
        finalNormal   ,              // Normal
        vec3(-20.0, 40.0, -15.0), // Light position
     ...
     directLight += directionalLight(
        ...
        1.0,                 // Light intensity,
        finalNormal  ,              // Normal
        vec3(-20.0, 40.0, -10.0), // Light position
        viewDirection,       // View direction
        30.0             // Specular power
    );
     directLight += directionalLight(
        ...
        vec3(10.0, 20.0, 80.0), // Light position
        viewDirection,       // View direction
        30.0             // Specular power
    );
    ...
    float mixStrength = (elevationSum + 0.375) * 0.3;
    mixStrength = smoothstep(-0.5, 0.5, mixStrength);

    vec3 elevationColor = mix(vec3( 0.68, 0.75, 0.78 ), vec3(0.9, 0.9, 0.9), mixStrength);


    // vec3 finalColor = color * light;
    vec3 finalColor = textureColor * elevationColor * totalLight;
    finalColor = smoothstep(-0.5, 0.6, finalColor);
